...
function borrowEquipments($selectedEquipment, $studentName, $studentId, $expectedReturn, $Department)
{
    if (!empty($selectedEquipment) && !empty($studentName) && !empty($studentId) && !empty($expectedReturn)) {
        ...
        try {
            ...
            $stmt = $conn->prepare("INSERT INTO smi_borrowed_tbl (smi_borrowed_equipmentName, smi_borrowed_studentName, smi_borrowed_studentId, smi_borrowed_department, smi_borrowed_dateBorrow, smi_borrowed_expReturn) VALUES (?, ?, ?, ?, NOW(), ?)");
            $stmt->execute([$equipmentString, $studentName, $studentId, $Department, $expectedReturn,]);
