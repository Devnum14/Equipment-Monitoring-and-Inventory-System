        ...
        if (!empty($selectedEquipment)) {
            ...
            try {
                ...
                foreach ($selectedEquipment as $equipment) {
                    ...
                    if ($Department === '') {
                        $stmt = $conn->prepare("INSERT INTO smi_borrowed_tbl (smi_borrowed_transid, smi_borrowed_equipmentName, smi_borrowed_qty, smi_borrowed_studentName, smi_borrowed_department, smi_borrowed_dateBorrow, smi_borrowed_expReturn) VALUES (?, ?, ?, ?, ?, NOW(), ?)");
                        $stmt->execute([$transvalue, $equipmentName, $quantity, $studentName, $Department, $expectedReturn,]);
                    } else {
                        $stmt = $conn->prepare("INSERT INTO smi_borrowed_tbl (smi_borrowed_transid, smi_borrowed_equipmentName, smi_borrowed_qty, smi_borrowed_studentName, smi_borrowed_department, smi_borrowed_dateBorrow, smi_borrowed_expReturn) VALUES (?, ?, ?, ?, ?, NOW(), ?)");
                        $stmt->execute([$transvalue, $equipmentName, $quantity, $studentName, $Department, $expectedReturn,]);
                    }
